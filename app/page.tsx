...
{uiToast && (
  <div className="animate-slide-up" style={{
    ...
    {uiToast}
      ...
      <div style={{ 
        position: 'fixed', 
        bottom: `calc(24px + env(safe-area-inset-bottom, 0px) + ${bannerHeight}px)`, 
        right: '20px', 
        zIndex: 99999, 
        display: 'flex', 
        flexDirection: 'column', 
        ...
        <button 
          ...
          style={{ width: '42px', height: '42px', backgroundColor: 'transparent', border: 'none', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: isDarkMode ? '#fafaf9' : '#1c1917', touchAction: 'none', userSelect: 'none', WebkitUserSelect: 'none' }} 
          title="Zoom In (hold for continuous)"
        >
          <Plus style={{ width: '18px', height: '18px', pointerEvents: 'none' }} />
        </button>
        <button 
          onPointerDown={(e) => startZoomHold(-1, e)}
          onPointerUp={stopZoomHold}
            ...
            <div className="animate-scale-up" style={{ backgroundColor: '#ffffff', borderRadius: '28px', boxShadow: '0 25px 50px -12px rgba(28, 25, 23, 0.35)', width: '100%', maxWidth: '440px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', padding: '24px', position: 'relative', boxSizing: 'border-box' }}>
              ...
              <div style={{ backgroundColor: '#fafaf9', border: '1px solid #e7e5e4', borderRadius: '18px', padding: '14px', marginBottom: '14px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{
                    backgroundColor: publicPassportStamps.some((s) => getStampTier(s.spotCount) === 'gold') ? '#fef3c7' : publicPassportStamps.some((s) => getStampTier(s.spotCount) === 'silver') ? '#f1f5f9' : '#e0f2fe',
                    color: publicPassportStamps.some((s) => getStampTier(s.spotCount) === 'gold') ? '#d97706' : publicPassportStamps.some((s) => getStampTier(s.spotCount) === 'silver') ? '#64748b' : '#0284c7',
                    fontSize: '10.5px',
                    fontWeight: 700,
                    padding: '2px 8px',
                    borderRadius: '6px'
  ...
  {isSavingUsername ? <Loader2 style={{ width: '15px', height: '15px', animation: 'spin 1s linear infinite' }} /> : profileSavedAt !== null ? <><Check style={{ width: '15px', height: '15px' }} /> Profile Saved!</> : 'Complete Profile'}
      ...
      {(isPassportBookOpen || isBookClosing) && (() => {
        ...
        return (
          <div
            className="animate-fade-in"
            style={{ position: 'fixed', inset: 0, zIndex: 100030, backgroundColor: 'rgba(28, 25, 23, 0.65)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', animation: isBookClosing ? 'fadeOut 0.24s cubic-bezier(0.16, 1, 0.3, 1) forwards' : undefined, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px', boxSizing: 'border-box' }}
            onTouchStart={(e) => { passportBookTouchStartRef.current = e.touches[0].clientX; }}
            onTouchEnd={(e) => {
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', maxWidth: '560px', padding: '16px 0 4px 0' }}>
              <button
                ...
                style={{ backgroundColor: '#1c1917', color: '#fafaf9', border: 'none', borderRadius: '12px', padding: '9px 16px', fontSize: '12px', fontWeight: 700, cursor: passportBookPage === 0 ? 'default' : 'pointer', opacity: passportBookPage === 0 ? 0.35 : 1 }}
              >
                ← Prev
              </button>
